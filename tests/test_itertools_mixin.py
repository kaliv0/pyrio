...
class TestItertoolsMixin:
    ...
    def test_chain(self):
        assert Stream("ABC").use(it.chain, iterables="DEF").to_list() == list(
            it.chain("ABC", "DEF")
        )
    ...
    def test_count(self):
        ...
        assert Stream.empty().use(it.count, start=10, step=2).limit(5).to_list() == [
            10,
            12,
            14,
            16,
    ...
    def test_itertools_dropwhile(self):
        coll = [1, 4, 6, 3, 8]
    ...
    def test_itertools_pairwise(self):
        letters = "ABCDEFG"
    ...
    def test_itertools_takewhile(self):
        coll = [1, 4, 6, 3, 8]
        predicate = lambda x: x < 5  # noqa
    ...
    def test_tee(self):
        coll = [1, 2, 3, 4, 5, 6]
        assert Stream(coll).use(it.tee, n=2).map(tuple).to_list() == [
            tuple(s) for s in it.tee(coll, 2)
        ]
    ...
    def test_take_nth_default_value(self):
        assert Stream.of(2, 3, 4).take_nth(10, default=66).get() == 66

    def test_take_nth_negative_index(self):
        assert Stream.of(2, 3, 4).take_nth(-1).get() == 4

    def test_take_nth_not_found(self):
    ...
    def test_all_equal_custom_key(self, Foo):
        fizz = Foo("fizz", 42)
        buzz = Foo("buzz", 42)
        coll = [fizz, buzz]
    ...
    def test_subslices_empty_collection(self):
        assert Stream.empty().subslices().to_list() == []

    def test_partition(self):
        assert Stream(range(10)).partition(lambda x: x % 2 != 0).map(
            lambda x: list(x)
        ).to_list() == [
            [1, 3, 5, 7, 9],
            [0, 2, 4, 6, 8],
    ...
    def test_grouper_default_incomplete(self):
    ...
    def test_grouper_default_fillvalue(self):
    ...
    def test_unique_reverse(self):
        assert Stream([[1, 2], [3, 4], [1, 2]]).unique(reverse=True).to_list() == [[3, 4], [1, 2]]

    def test_unique_custom_key(self, Foo):
        foo = Foo("foo", 1)
        bar = Foo("bar", 2)
        fizz = Foo("fizz", 3)
        buzz = Foo("buzz", 4)
    ...
    def test_unique_custom_key_reversed(self, Foo):
        foo = Foo("foo", 1)
        bar = Foo("bar", 2)
        fizz = Foo("fizz", 3)
        buzz = Foo("buzz", 4)
    ...
    def test_unique_just_seen(self):
        assert Stream("AAAABBBCCDAABBB").unique_just_seen().to_list() == [
            "A",
            "B",
            "C",
            "D",
    ...
    def test_unique_just_seen_custom_key(self):
    ...
    def test_unique_just_seen_empty_collection(self):
        assert Stream([]).unique_just_seen().to_list() == []

    def test_unique_ever_seen(self):
        assert Stream("AAAABBBCCDAABBB").unique_ever_seen().to_list() == ["A", "B", "C", "D"]

    def test_unique_ever_seen_custom_key(self):
        assert Stream("ABBcCAD").unique_ever_seen(key=str.casefold).to_list() == [
            "A",
            "B",
            "c",
            "D",
    ...
    def test_find_indices_custom_start(self):
        assert Stream("AABCADEAF").find_indices(value="A", start=3).to_list() == [4, 7]

    def test_find_indices_custom_stop(self):
