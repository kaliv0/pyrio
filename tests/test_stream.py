...
class TestStream:
    ...
    def test_stream_from_none(self):
    ...
    def test_stream_of_nullable(self):
    ...
    def test_iterable_from_string(self):
        json_str = (
            '{"Name": "Jennifer Smith", "Phone": "555-123-4568", "Email": "jen123@gmail.com"}'
        )
        json_map = json.loads(json_str)
        assert Stream(json_map).filter(lambda x: len(x.key) < 6).map(
            lambda x: x.key
        ).to_tuple() == (
            "Name",
            "Phone",
    ...
    def test_empty_json_from_string(self):
        empty_json = "{}"
    ...
    def test_map_dict(self):
        assert Stream({"x": 1, "y": 2}).map(lambda x: x.key + str(x.value)).to_list() == [
            "x1",
            "y2",
        ]

    def test_filter_map(self):
        assert Stream([None, "foo", "", "bar"]).filter_map(str.upper).to_list() == [
            "FOO",
            "",
            "BAR",
        ]

    def test_filter_map_discard_falsy(self):
    ...
    def test_skip_bigger_than_stream_count(self):
    ...
    def test_limit_bigger_than_stream_count(self):
    ...
    def test_tail_bigger_than_stream_count(self):
    ...
    def test_concat_linear_collections(self):
    ...
    def test_concat_dicts_to_stream(self):
        first_dict = {"x": 1, "y": 2}
    ...
    def test_concat_raises_non_iterable(self):
    ...
    def test_flat_map(self):
        assert Stream([[1, 2], [3, 4], [5]]).flat_map(lambda x: Stream(x)).to_list() == [
            1,
            2,
            3,
            4,
    ...
    def test_flatten_no_nested_levels(self):
        assert Stream([1, 2]).flatten().to_list() == [1, 2]
    ...
    def test_sum_empty_collection(self):
        assert Stream([]).sum() == 0
        assert Stream.empty().sum() == 0

    def test_sum_non_number_elements(self):
        with pytest.raises(ValueError) as e:
            Stream.of("a", "b").sum()
        assert str(e.value) == "Cannot apply sum on non-number elements"

    def test_sum_on_iterator(self):
    ...
    def test_sum_mixed_types_raises(self):
        with pytest.raises(ValueError) as e:
    ...
    def test_average_mixed_types_raises(self):
        with pytest.raises(ValueError) as e:
    ...
    def test_take_while_no_elements(self):
        assert (
    ...
    def test_drop_while_no_elements(self):
        assert Stream.of("adam", "aman", "ahmad", "hamid", "muhammad", "aladdin").drop_while(
            lambda x: x[0] == "xyz"
    ...
    def test_take_last_on_iterator(self):
        assert Stream(iter([1, 2, 3, 4, 5])).take_last().get() == 5

    def test_take_last_on_empty_iterator(self):
        assert Stream(iter([])).take_last().is_empty()

    def test_take_last_on_empty_iterator_with_default(self):
        assert Stream(iter([])).take_last(default=99).get() == 99

    def test_take_last_on_single_element_iterator(self):
    ...
    def test_sort_comparator_function(self):
    ...
    def test_sort_comparator_and_reverse(self):
        assert Stream.of(3, 5, 2, 1).map(lambda x: (str(x), x * 10)).sort(
    ...
    def test_reverse_with_custom_comparator(self):
    ...
    def test_stream_on_close_callback(self):
        f = io.StringIO()
        with redirect_stdout(f):
            result = (
                Stream([1, 2, 3, 4])
    ...
    def test_stream_on_close_callback_using_pointer_to_self(self):
        flag = False
    ...
    def test_compare_with_custom_key(self, Foo):
        fizz = Foo("fizz", 1)
        buzz = Foo("buzz", 2)
    ...
    def test_quantify_default_predicate(self):
    ...
    def test_find_first_with_predicate(self):
        assert Stream.of(1, 2, 3, 4).find_first(lambda x: x % 2 == 0).get() == 2

    def test_find_first_in_empty_stream(self):
    ...
    def test_find_any_with_predicate(self):
        assert Stream.of(1, 2, 3, 4).find_any(lambda x: x % 2 == 0).get() in (2, 4)

    def test_find_any_in_empty_stream(self):
    ...
    def test_none_match_false(self):
    ...
    def test_to_dict_merger(self, Foo):
    ...
    def test_to_dict_duplicate_key_no_merger_raises(self, Foo):
        coll = [Foo("fizz", 1), Foo("fizz", 2), Foo("buzz", 2)]
        with pytest.raises(IllegalStateError) as e:
            Stream(coll).to_dict(
    ...
    def test_to_dict_no_collector(self, Foo):
        first_dict = {"x": 1, "y": 2}
        second_dict = {"p": 33, "q": 44, "r": None}
    ...
    def test_collect_to_dict_raises(self, Foo):
        coll = [Foo("fizz", 1), Foo("fizz", 2), Foo("buzz", 2)]
    ...
    def test_collect_invalid_type(self, Foo):
        coll = [Foo("fizz", 1), Foo("fizz", 2), Foo("buzz", 2)]
        with pytest.raises(ValueError) as e:
    ...
    def test_group_by_custom_collector(self):
    ...
    def test_group_by_objects(self, Foo):
        coll = [
            Foo("fizz", 1),
            Foo("fizz", 2),
            Foo("fizz", 3),
    ...
    def test_group_by_unconsumed_groups(self):
        stream = Stream("AAABBB")
        keys = [key for key, group in stream._group_by()]
        assert keys == ["A", "B"]

    def test_to_string(self, nested_json):
    ...
    def test_repr(self, nested_json):
    ...
    # ### nested streams ###
    def test_nested_json_from_string(self, nested_json):
        assert (
    ...
    def test_nested_json_querying_nested_dict_items(self, nested_json):
        assert (
            Stream(json.loads(nested_json))
