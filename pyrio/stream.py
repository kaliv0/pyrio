...
class Stream:
    ...
    def use(self, it_function, *args, **kwargs):
        ...
        return self._handle_default_signature_functions(it_function, kwargs, signature)

    def _handle_no_signature_functions(self, it_function, **kwargs):
        NO_SIGNATURE_FUNCTIONS = ["chain", "islice", "product", "repeat"]
    ...
    def _handle_no_kwargs_functions(self, signature, it_function, **kwargs):
        NO_KWARGS_FUNCTIONS = ["dropwhile", "filterfalse", "starmap"]
        ...
        if it_function.__name__ in NO_KWARGS_FUNCTIONS:
            # self._iterable = it_function(kwargs.get("function", None), self._iterable)
            # TODO: starmap uses function instead of predicate
            self._iterable = it_function(*kwargs.values(), self._iterable)
            return True
        return False

    def _handle_default_signature_functions(self, it_function, kwargs, signature):
